BEGIN {
	...
	MAX_WIDTH = 80
	if (ENVIRON["RST_MAX_WIDTH"] ~ /^[0-9]+$/) {
		if (int(ENVIRON["RST_MAX_WIDTH"]) < MAX_WIDTH) {
			MAX_WIDTH = int(ENVIRON["RST_MAX_WIDTH"])
		}
	}
...
function printverse(verse,    word_count, characters_printed) {
	if (ENVIRON["RST_NOLINEWRAP"] != "" && ENVIRON["RST_NOLINEWRAP"] != "0") {
		printf("%s\n", verse)
		return
	}
